...
(async () => {
  ...
  console.log('Solving first part');
  const beforeFirst = performance.now();
  console.log(first(input));
  console.log(`Took ${performance.now() - beforeFirst}ms`);
  console.log('Solving second part');
  const beforeSecond = performance.now();
  console.log(second(input));
  console.log(`Took ${performance.now() - beforeSecond}ms`);
})();
